...
useEffect(() => {
  ...
  if(storedUserLoggedInfo==='1'){ // бул жерде кадимки эле условия эгер локалда бир данный бар болчу болсо же данный келсе андаа setIsLoggedIn(true) болот 
    setIsLoggedIn(true)
  }
}, [])

  const loginHandler = async(email, password) => {// бул функция баласынан атасына данныйларт алып чыгып берет мындайча айтканда логин деген компоненттен бизге инпутка жазылган данныйларды
  ...
  const logoutHandler = () => { // функция бизде бир баттон бар ошого пропс аркылуу берилген ал баттон басылса локалды тазалап салат жана setIsLoggedIn(false) кылат ал false болсо бизде кайра логин коргозуп койот
